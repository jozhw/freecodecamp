...
echo "$($PSQL "SELECT DISTINCT name FROM (SELECT winner_id AS team_id FROM games WHERE year=2014 AND round='Eighth-Final' UNION ALL SELECT opponent_id AS team_id FROM games WHERE year=2014 AND round='Eighth-Final') AS games INNER JOIN teams USING(team_id) ORDER BY name;")"
